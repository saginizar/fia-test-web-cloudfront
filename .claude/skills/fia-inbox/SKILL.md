...
1. If `Intelligent-Feedback-Agent-FIA/fia.config.json` exists at the repo root,
   that's one entry (standard-mode install).
2. For every sibling folder at the repo root that has its own
   `<name>/fia.config.json`, check whether `<name>/fia.owner.local.json` also
   exists — if so, that's another entry (monorepo-mode install for that skill).
...
```bash
curl.exe -sS "<api_base>/skill/summary?tool_id=<tool_id>&kit_version=<kit_version>" \
